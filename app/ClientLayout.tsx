      ...
      {openMegaMenu && (() => {
        ...
        return (
          <div className={`hidden md:block absolute top-full left-1/2 -translate-x-1/2 w-full transition-[max-width,padding] duration-500 ease-out ${scrolled ? "pt-2 px-3 md:px-6 max-w-3xl" : "pt-0 max-w-[1600px]"}`} style={{ animation: "megaReveal 0.34s cubic-bezier(0.16,1,0.3,1)" }}>
            ...
            <div className={`backdrop-blur-2xl origin-top transition-[border-radius,border-color,box-shadow,background-color] duration-500 ease-out ${isLightPage ? "bg-[#f5f3f0]/75" : "bg-[#0c0c0c]/70"} ${scrolled ? (isLightPage ? "rounded-3xl border border-black/[0.07] overflow-hidden shadow-[0_40px_90px_-20px_rgba(0,0,0,0.25)]" : "rounded-3xl border border-white/[0.07] overflow-hidden shadow-[0_40px_90px_-20px_rgba(0,0,0,0.95)]") : (isLightPage ? "rounded-b-[28px] border-b border-x border-black/[0.08] overflow-hidden shadow-[0_40px_80px_-24px_rgba(0,0,0,0.2)]" : "rounded-b-[28px] border-b border-x border-white/10 overflow-hidden shadow-[0_40px_80px_-24px_rgba(0,0,0,0.9)]")}`}>
