    ...
    <div className="w-full flex-1 bg-[#f5f3f0] text-[#131313] min-h-screen">
      ...
      <nav className="md:hidden fixed inset-x-3 mx-auto max-w-md bottom-[max(0.75rem,env(safe-area-inset-bottom))] z-[92] p-1.5 rounded-full border border-[#e2e0dc] bg-white/90 backdrop-blur-2xl shadow-[0_18px_44px_-14px_rgba(0,0,0,0.26)] grid grid-cols-5">
        ...
        <button onClick={() => { setView("home"); setQuery(""); window.scrollTo({ top: 0, behavior: "smooth" }); }}
          ...
          <svg className="w-[19px] h-[19px]" fill="none" viewBox="0 0 24 24" strokeWidth={1.6} stroke="currentColor">
        ...
        <button onClick={() => setShowWishList(true)} aria-label="찜한 상품"
          ...
          <svg className="w-[19px] h-[19px]" fill={showWishList ? "currentColor" : "none"} viewBox="0 0 24 24" strokeWidth={1.6} stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
          </svg>
          {wish.length > 0 && (
        ...
        {isLoggedIn ? (
          <Link href="/shop/me" aria-label="내 정보"
            className="relative flex items-center justify-center py-2 rounded-full text-[#8a8a8a] active:text-[#131313] transition-all active:scale-95">
            {/* 고급 이글루 하단바와 같은 아이콘을 쓴다 */}
            <svg className="w-[19px] h-[19px]" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
            </svg>
            {pendingOrders > 0 && <span className="absolute top-1 right-1/2 translate-x-3.5 w-2 h-2 rounded-full bg-[#e91e3f] ring-2 ring-white"></span>}
          </Link>
        ) : (
