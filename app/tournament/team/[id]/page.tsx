...
type Fixture = { _id: string; teamAId: string; teamBId: string; kind?: string; at: string; winnerId: string; scoreA: number; scoreB: number; mercs?: number; mercsA?: number; mercsB?: number };
                    ...
                    return (
                      <div key={f._id} className="esp-cut border border-white/[0.08] bg-white/[0.02] mb-2.5">
                        <div className="px-5 py-2.5 flex items-center gap-2 border-b border-white/[0.07]">
                          ...
                          {(() => {
                            // 팀마다 용병 수가 다르다 — 우리 기준으로 적는다 (옛 공통 값도 함께 읽는다)
                            const us = (f.teamAId === id ? f.mercsA : f.mercsB) || f.mercs || 0;
                            const them = (f.teamAId === id ? f.mercsB : f.mercsA) || f.mercs || 0;
                            if (us + them === 0) return null;
                            return (
                        ...
                        <div key={f._id} className="flex items-center gap-3 py-3 border-b border-white/[0.06]">
                          ...
                          <span className="flex-1 min-w-0 text-[12px] font-black text-gray-300 truncate">
                            ...
                            {(() => {
                              const us = (f.teamAId === id ? f.mercsA : f.mercsB) || f.mercs || 0;
                              const them = (f.teamAId === id ? f.mercsB : f.mercsA) || f.mercs || 0;
                              return us + them > 0 ? <span className="ml-1.5 text-[9px] font-black esp-mono text-sky-300/70">용병 {us}·{them}</span> : null;
                            })()}
                          </span>
