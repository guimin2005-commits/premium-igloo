  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        href: "/admin/shop",
        children: [
          { title: "아이템 등록", href: "/admin/shop?tab=items" },
          { title: "상품 관리", href: "/admin/shop?tab=products" },
  ...
  const linkClass = (active: boolean, child = false) =>
    `relative flex items-center gap-2.5 px-3 py-2 rounded-lg font-bold transition-colors ${child ? "text-[12px] ml-4" : "text-[13px]"} ${
      active
        ...
        : "text-[#5a5a5a] hover:text-[#131313] hover:bg-black/[0.06]"
    }`;

  return (
    <>
      {/* ── 데스크톱: 좌측 사이드 패널 ── */}
      <aside className={`hidden lg:block w-60 shrink-0 border-r ${isDark ? "border-white/10 bg-[#0e0e0e]" : "border-black/10 bg-[#f2f2f2]"}`}>
      ...
      <div className={`lg:hidden w-full border-b ${isDark ? "border-white/10 bg-[#0e0e0e]" : "border-black/10 bg-[#f2f2f2]"}`}>
              ...
              <Link
                ...
                className={`shrink-0 px-3.5 py-1.5 rounded-full text-[11px] font-bold border transition-colors ${
                  active
                    ...
                    : "text-[#5a5a5a] border-black/20 hover:text-[#131313] hover:border-black/40"
                }`}
              >
                {item.title}
              </Link>
