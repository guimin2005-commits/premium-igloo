      ...
      <main key={viewMode} className="flex-1 w-full max-w-5xl mx-auto px-5 md:px-8 pt-10 md:pt-12 pb-24 md:pb-16 text-[#131313]">
        <div className="flex items-end justify-between gap-4 mb-5">
          ...
          <button onClick={() => setViewMode("user")}
            className="h-9 px-4 rounded-full bg-[#f2f2f2] hover:bg-[#e0e0e0] text-[#4b4b4b] text-[12px] font-extrabold transition-colors shrink-0">구인으로</button>
        </div>
        ...
        {isLoadingApplies ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-6">
            {applies.map((app) => {
              ...
              const stCls = st === "합격" ? "text-[#131313]" : (st === "불합격" || st === "취소" || st === "취소/반려") ? "text-[#a3a3a3]" : "text-[#e91e3f]";
              return (
                <div key={app._id} className="border border-[#ededed] rounded-2xl bg-white p-6 flex flex-col">
                  ...
                  <div className="grid grid-cols-2 gap-4 py-5">
                    ...
                    <div>
                      ...
                      <span className="text-[14px] font-extrabold text-[#4b4b4b] tabular-nums">{app.age}세</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-5 pb-6">
                    <div>
                      ...
                      <p className="text-[14px] text-[#4b4b4b] whitespace-pre-wrap leading-[1.8] break-keep">{app.intro}</p>
                    </div>
                    {app.experience && (
                      <div>
                        <span className="block text-[11px] font-black text-[#8a8a8a] mb-2">관련 경험</span>
                        <p className="text-[14px] text-[#4b4b4b] whitespace-pre-wrap leading-[1.8] break-keep">{app.experience}</p>
                      </div>
                    )}
                  </div>

                  <div className="mt-auto flex gap-2 pt-5 border-t border-[#ededed]">
                    <button onClick={() => handleUpdateStatus(app._id, "합격")} className="flex-1 h-10 rounded-full bg-[#131313] hover:bg-black text-white text-[12px] font-extrabold transition-colors">합격</button>
                    <button onClick={() => handleUpdateStatus(app._id, "불합격")} className="flex-1 h-10 rounded-full bg-[#f2f2f2] hover:bg-[#e0e0e0] text-[#4b4b4b] text-[12px] font-extrabold transition-colors">불합격</button>
                    <button onClick={() => setApplyDeleteConfirmId(app._id)} className="flex-1 h-10 rounded-full border border-[#e0e0e0] text-[#e91e3f] hover:bg-[#e91e3f] hover:text-white hover:border-[#e91e3f] text-[12px] font-extrabold transition-colors">삭제</button>
                  </div>
                </div>
              );
            })}
        ...
        {applyDeleteConfirmId && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={() => setApplyDeleteConfirmId(null)}>
            <div className="bg-white rounded-3xl w-full max-w-sm p-7 text-center border border-[#e0e0e0] shadow-2xl" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex gap-2">
                <button onClick={() => setApplyDeleteConfirmId(null)} className="flex-1 py-3 rounded-xl bg-[#f2f2f2] hover:bg-[#e0e0e0] text-[#4b4b4b] text-[13px] font-bold transition-colors">취소</button>
                <button onClick={executeApplyDelete} className="flex-1 py-3 rounded-xl bg-[#e91e3f] hover:bg-[#d01634] text-white text-[13px] font-bold transition-colors">삭제</button>
              </div>
            </div>
          </div>
        )}
          ...
          {isAdmin ? (
            <div className="flex items-center gap-2 shrink-0">
              <button onClick={() => setViewMode("admin")}
                className="h-9 px-4 rounded-full bg-[#f2f2f2] hover:bg-[#e0e0e0] text-[#4b4b4b] text-[12px] font-extrabold transition-colors">지원자</button>
              <button onClick={() => router.push("/write?category=구인")}
                className="h-9 px-4 rounded-full bg-[#131313] hover:bg-black text-white text-[12px] font-extrabold transition-colors">구인글 작성</button>
            </div>
          ) : (
              ...
              return (
                <div key={post._id} onClick={() => router.push(`/recruit/${post._id}`)}
                  ...
                  <span className="flex-1 min-w-0">
                    ...
                    <span className={`block text-[15px] font-extrabold leading-snug truncate transition-colors ${isEnded ? "text-[#6a6a6a]" : "group-hover:text-[#e91e3f]"}`}>{post.title}</span>
                  ...
                  <span className="flex items-center gap-3 shrink-0">
                    {isAdmin && (
                      <span className="hidden md:flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); setDeleteConfirmId(post._id); }} className="text-[11px] font-bold text-[#e91e3f] hover:text-[#c62828] transition-colors">삭제</button>
                      </span>
                    )}
