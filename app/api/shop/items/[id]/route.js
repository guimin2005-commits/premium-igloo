...
import { itemEffectLines, describeRoleBuff } from "@/lib/itemEffects";
...
//    📌 effectLines — 사면 붙는 효과 문장. 인벤토리(my-items)와 같은 문장이다:
...
export async function GET(request, { params }) {
  try {
    ...
    try {
      ...
      effectLines = [...itemEffectLines(linked, (cid) => names.get(cid)), ...describeRoleBuff(cfg || {})];
    } catch (e) {
      // 효과 문장이 없다고 상품을 못 보면 안 된다
      console.error("상품 효과 문장 오류:", e);
    }

    return NextResponse.json({ success: true, data: { ...item, effectLines } });
  } catch (e) {
