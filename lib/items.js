...
// 📌 설명 — 줄바꿈(\n)을 살린다("안녕하세요.\n저는 펭귄이에요."). 아이템 · 상품 설명이 같은 규칙을 쓴다.
//    CRLF → LF, 줄 끝 공백 제거, 빈 줄은 연속 2줄까지, 앞뒤 공백 trim, 300자(UTF-16 기준 — 스키마 maxlength 와 같은 셈).
...
export function normalizeItemPayload(b) {
  ...
  // 순서 — 비우거나 안 보내면 넣지 않는다(수정은 기존 값 유지 · 새 아이템은 라우트가 맨 뒤로). 끌어서 정하는 게 기본이다
  const hasSort = b?.sortOrder !== "" && b?.sortOrder != null && Number.isFinite(Number(b.sortOrder));
  return {
    ok: true,
    data: {
      name,
      ...
      ...(hasSort ? { sortOrder: Math.max(-999, Math.min(999, Math.round(Number(b.sortOrder)))) } : {}),
    },
  };
}
