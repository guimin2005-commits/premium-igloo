...
export const couponError = (coupon, total, userId) => {
  ...
  // 보상형은 쿠폰함에서 등록하면 바로 지급된다 — 결제 코드로 받으면 할인 0 으로 통과해 보상만 소진된다
  if (coupon.kind === "reward") return "쿠폰함에서 등록하는 쿠폰입니다.";
...
// 📌 쿠폰 사용권 선점 조건 — 한도 검사와 사용 기록({ $inc: usedCount, $push: usedBy })을 조건부 갱신 한 번으로 묶는다.
//    읽고 검사한 뒤 따로 기록하면 동시 요청이 모두 검사를 통과해 한도를 넘긴다. 한도는 couponError 와 같이 읽어 온 쿠폰 값으로 건다
export const couponClaimFilter = (coupon, userId) => {
  const and = [];
  if (coupon.maxUses > 0) and.push({ $expr: { $lt: [{ $ifNull: ["$usedCount", 0] }, coupon.maxUses] } });
  if (coupon.perUserLimit > 0) {
    const mine = { $size: { $filter: { input: { $ifNull: ["$usedBy", []] }, cond: { $eq: ["$$this", { $literal: userId }] } } } };
    and.push({ $expr: { $lt: [mine, coupon.perUserLimit] } });
  }
  return { _id: coupon._id, active: true, ...(and.length ? { $and: and } : {}) };
};

// 선점 되돌리기 — usedBy 에서 이 유저를 한 번만 뺀다($pull 은 1인 여러 회 쿠폰의 앞선 사용 기록까지 지운다).
//    파이프라인 갱신이라 { updatePipeline: true } 와 함께 쓴다
export const couponReleaseUpdate = (userId) => [{
  $set: {
    usedCount: { $max: [0, { $subtract: [{ $ifNull: ["$usedCount", 0] }, 1] }] },
    usedBy: {
      $let: {
        vars: { list: { $ifNull: ["$usedBy", []] } },
        in: {
          $let: {
            vars: { i: { $indexOfArray: ["$$list", { $literal: userId }] } },
            in: {
              $cond: [
                { $lt: ["$$i", 0] },
                "$$list",
                { $concatArrays: [{ $slice: ["$$list", "$$i"] }, { $slice: ["$$list", { $add: ["$$i", 1] }, { $size: "$$list" }] }] },
              ],
            },
          },
        },
