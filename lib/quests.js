...
export async function getQuestState(userId) {
  ...
  const [quests, buckets, claims, user, setting] = await Promise.all([
    ...
    //       그래서 둘 중 이른 쪽부터 읽고, 월간도 조건 합산으로 센다.
    XpLog.aggregate([
  ...
  // ── 내장 출석 퀘스트 — 음성 접속 누적 N분 또는 디스코드 /출석체크 (항상 일일 · 둘 합쳐 하루 1회) ──
  ...
  const attendQuest = {
    ...
    period: "daily",
    name: "출석",
    desc: `음성 ${targetMin}분을 채우거나 /출석체크 — 하루 한 번`,
    reason: "voice",
    metric: "minute",
    target: targetMin,
    current: Math.min(voiceMin, targetMin),
    rewardXp: attendXp,
    // 출석은 POINT 를 주지 않지만 필드는 채워 둔다 — 화면이 퀘스트마다 필드 유무를 따지지 않게
    rewardPoint: 0,
    // /출석체크 로 받은 날은 음성 시간을 못 채웠어도 끝난 것으로 본다
    done: attendDone || attendClaimed,
    claimed: attendClaimed,
    // 출석은 눌러서 받는 게 아니다 — 봇이 기준 시간을 채우는 순간 자동으로 준다.
    //    그래서 수령 버튼을 띄우지 않고 진행 상황만 보여준다.
    auto: true,
    claimable: false,
  };
  ...
  const rows = selected.map((q) => {
    ...
    // 화면에 적는 POINT 는 배율 적용 전 기본값이다 — 실지급액은 수령 응답(claimed.point)이 알려 준다
