...
async function seasonStartBaseXp(userId, xp) {
...
export async function getPassState(userId) {
  ...
  if (user && seen !== SEASON.number) {
    ...
    const startBase = await seasonStartBaseXp(userId, xp);
  ...
  const itemIds = [...new Set(tiers.flatMap((t) => [t.free.itemId, t.paid.itemId]).filter(Boolean))];
...
export async function grantReward(userId, reward, label, userName = "", level = 0) {
  ...
  if (kind === "item") {
    ...
    // 아이템이 지워졌으면 던진다 — 호출부(claim)가 수령 자물쇠를 되돌려 관리자가 고친 뒤 다시 받을 수 있게 한다
