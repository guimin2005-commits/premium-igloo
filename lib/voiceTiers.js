...
// 📌 point = 그 등급으로 오를 때 주는 빙옥. 1 빙옥 = 1,000 XP 로 바뀌면서(2026-09) 다시 정했다 —
//    "그 등급까지 올라오는 데 쓴 XP(직전 등급부터의 누적 XP)의 약 10% 를 빙옥으로". 예: 이글루 구간 2,903만 XP → 2,900 빙옥.
export const VOICE_TIERS = [
  { key: "iron",        name: "아이언",       en: "IRON",        min: 1,   bonus: 0,    point: 0,    c: "#8a8a8a" },
  { key: "bronze",      name: "브론즈",       en: "BRONZE",      min: 90,  bonus: 300,  point: 50,   c: "#a06a3c" },
  { key: "silver",      name: "실버",         en: "SILVER",      min: 160, bonus: 550,  point: 120,  c: "#8d99a6" },
  { key: "gold",        name: "골드",         en: "GOLD",        min: 240, bonus: 750,  point: 250,  c: "#c39220" },
  { key: "platinum",    name: "플래티넘",     en: "PLATINUM",    min: 330, bonus: 1100, point: 450,  c: "#3f9e93" },
  { key: "diamond",     name: "다이아몬드",   en: "DIAMOND",     min: 430, bonus: 1500, point: 750,  c: "#3f7fc4" },
  { key: "master",      name: "마스터",       en: "MASTER",      min: 540, bonus: 1900, point: 1200, c: "#8557b0" },
  { key: "grandmaster", name: "그랜드마스터", en: "GRANDMASTER", min: 650, bonus: 2200, point: 1600, c: "#a03a6b" },
  { key: "challenger",  name: "챌린저",       en: "CHALLENGER",  min: 760, bonus: 2500, point: 2100, c: "#c2452f" },
  // 최고 티어는 커뮤니티 이름 그대로 — 여기까지 오면 이글루 그 자체
  { key: "igloo",       name: "이글루",       en: "IGLOO",       min: 880, bonus: 3000, point: 2900, c: "#e91e3f" },
];

export const TIER_COLORS = VOICE_TIERS.map((t) => t.c);

// 레벨 → 티어 인덱스
// 📌 승급 보상 포인트 — 아이언에서 시작해 등급이 오를 때마다 그 등급의 point 를 준다.
//    from 보다 위로 올라간 등급들의 몫을 모두 합산한다 (한 번에 여러 등급을 건너뛰어도 빠지지 않게).
export const tierPointsBetween = (fromIdx, toIdx) => {
  let sum = 0;
  for (let i = Math.max(0, fromIdx) + 1; i <= toIdx && i < VOICE_TIERS.length; i++) sum += VOICE_TIERS[i].point || 0;
  return sum;
};
